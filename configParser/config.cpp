...
vector<pair<string, string> >& Config::getTemplates(string prefix) {
	templates.clear();
	for (map<string, string>::iterator i = symbols.begin(); i != symbols.end(); ++i) {
		string varName = i->first;
		string varContent = i->second;
		if(varName.find(prefix) == 0){
			string templateName = varName.substr(prefix.length());
			templates.push_back(pair<string,string>(templateName, varContent));
		}
	}
