...
void TTM_TMVASampler::FillTree(TTree* iTree, Process const * iProcess){
	...
	for(unsigned int i = 0; i < goodEventsForSignal.size(); i++){
		...
		float tau1eta	= event->TTM_Tau1Eta->at(bestCombo);
		float tau1phi	= event->TTM_Tau1Phi->at(bestCombo);
		float tau2eta	= event->TTM_Tau2Eta->at(bestCombo);
		float tau2phi	= event->TTM_Tau2Phi->at(bestCombo);
		float lepEta	= event->TTM_MuonEta->at(bestCombo);
		float lepPhi	= event->TTM_MuonPhi->at(bestCombo);
