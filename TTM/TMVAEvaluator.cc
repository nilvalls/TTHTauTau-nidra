...
float TTM_TMVAEvaluator::Evaluate(TTMBranches const * iEvent, int iCombo){
    ...
    Tau2Pt = iEvent->TTM_Tau2Pt->at(iCombo);
    Tau1DecayMode = iEvent->TTM_Tau1DecayMode->at(iCombo);
    Tau2DecayMode = iEvent->TTM_Tau2DecayMode->at(iCombo);
    Tau1IsolationIndex = iEvent->GetTau1IsolationIndex(iCombo);
    Tau2IsolationIndex = iEvent->GetTau2IsolationIndex(iCombo);
    Tau1LTPt = iEvent->TTM_Tau1LTPt->at(iCombo);
    Tau2LTPt = iEvent->TTM_Tau2LTPt->at(iCombo);
    Tau1NProngs = iEvent->TTM_Tau1NProngs->at(iCombo);
    Tau2NProngs = iEvent->TTM_Tau2NProngs->at(iCombo);
    DitauVisibleMass = iEvent->TTM_DitauVisibleMass->at(iCombo);

    float tau1eta	= iEvent->TTM_Tau1Eta->at(iCombo);
    float tau1phi	= iEvent->TTM_Tau1Phi->at(iCombo);
    float tau2eta	= iEvent->TTM_Tau2Eta->at(iCombo);
    float tau2phi	= iEvent->TTM_Tau2Phi->at(iCombo);
    float lepEta	= iEvent->TTM_MuonEta->at(iCombo);
    float lepPhi	= iEvent->TTM_MuonPhi->at(iCombo);
