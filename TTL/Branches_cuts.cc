...
void
TTLBranches::RegisterCuts(CutFlow& c)
{
    ...
    c.RegisterCut("T1_MatchAbsId", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return abs((*e->TTL_Tau1GenMatchId)[idx]); }, true);
    c.RegisterCut("T2_MatchAbsId", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return abs((*e->TTL_Tau2GenMatchId)[idx]); }, true);
    c.RegisterCut("T1_ParentAbsId", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return abs((*e->TTL_Tau1GenMatchMother0Id)[idx]); }, true);
    c.RegisterCut("T2_ParentAbsId", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
    ...
    c.RegisterCut("L_pT", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_LeptonPt)[idx]; });
    c.RegisterCut("L_Eta", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_LeptonEta)[idx]; });
    c.RegisterCut("L_IsLoose", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_LeptonIsLoose)[idx]; });
    c.RegisterCut("L_IsTight", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_LeptonIsTight)[idx]; });
    c.RegisterCut("L_RelIso", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_LeptonRelIso)[idx]; });

    c.RegisterCut("T1_pT", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1Pt)[idx]; });
    c.RegisterCut("T2_pT", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2Pt)[idx]; });
    c.RegisterCut("T1_Eta", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1Eta)[idx]; });
    c.RegisterCut("T2_Eta", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2Eta)[idx]; });
    c.RegisterCut("T1_InCracks", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1IsInTheCracks)[idx]; });
    c.RegisterCut("T2_InCracks", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2IsInTheCracks)[idx]; });

    c.RegisterCut("TT_DeltaR", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_DitauDeltaR)[idx]; });
    c.RegisterCut("T1L_DeltaR", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return DeltaR((*e->TTL_Tau1Eta)[idx], (*e->TTL_Tau1Phi)[idx],
                (*e->TTL_LeptonEta)[idx], (*e->TTL_LeptonPhi)[idx]); });
    c.RegisterCut("T2L_DeltaR", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return DeltaR((*e->TTL_Tau2Eta)[idx], (*e->TTL_Tau2Phi)[idx],
                (*e->TTL_LeptonEta)[idx], (*e->TTL_LeptonPhi)[idx]); });

    c.RegisterCut("T1_DecayModeFinding", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1HPSdecayModeFinding)[idx]; });
    c.RegisterCut("T2_DecayModeFinding", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2HPSdecayModeFinding)[idx]; });
    c.RegisterCut("T1_LTvalid", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1LTvalid)[idx]; });
    c.RegisterCut("T2_LTvalid", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2LTvalid)[idx]; });
    c.RegisterCut("T1_LTpT", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1LTPt)[idx]; });
    c.RegisterCut("T2_LTpT", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2LTPt)[idx]; });
    c.RegisterCut("T1_NumProngs", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1NProngs)[idx]; });
    c.RegisterCut("T2_NumProngs", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2NProngs)[idx]; });

    c.RegisterCut("T1_AntiElectronMVA", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1HPSagainstElectronMVA)[idx]; });
    c.RegisterCut("T2_AntiElectronMVA", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2HPSagainstElectronMVA)[idx]; });
    c.RegisterCut("T1_AntiElectronIndex", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau1AntiElectronIndex(idx); });
    c.RegisterCut("T2_AntiElectronIndex", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau2AntiElectronIndex(idx); });
    c.RegisterCut("T1_AntiMuonIndex", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau1AntiMuonIndex(idx); });
    c.RegisterCut("T2_AntiMuonIndex", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau2AntiMuonIndex(idx); });

    c.RegisterCut("T1_IsolationIndex", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau1IsolationIndex(idx); });
    c.RegisterCut("T2_IsolationIndex", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau2IsolationIndex(idx); });
    c.RegisterCut("T1_IsolationIndex3Hits", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau1IsolationIndex3Hits(idx); });
    c.RegisterCut("T2_IsolationIndex3Hits", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau2IsolationIndex3Hits(idx); });
    c.RegisterCut("T1_IsolationIndexMVA", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau1IsolationIndexMVA(idx); });
    c.RegisterCut("T2_IsolationIndexMVA", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau2IsolationIndexMVA(idx); });
    c.RegisterCut("T1_IsolationIndexMVA2", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau1IsolationIndexMVA2(idx); });
    c.RegisterCut("T2_IsolationIndexMVA2", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return e->GetTau2IsolationIndexMVA2(idx); });
    c.RegisterCut("T1_IsolationMVA2Raw", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1HPSbyIsolationMVA2raw)[idx]; });
    c.RegisterCut("T2_IsolationMVA2Raw", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau2HPSbyIsolationMVA2raw)[idx]; });

    c.RegisterCut("TT_ChargeProduct", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1Charge)[idx] * (*e->TTL_Tau2Charge)[idx]; });

    c.RegisterCut("TT_VisMass", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_DitauVisibleMass)[idx]; });
    c.RegisterCut("TT_DeltaEta", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
            return (*e->TTL_Tau1Eta)[idx] - (*e->TTL_Tau2Eta)[idx]; });
    c.RegisterCut("TT_CosDeltaPhi", 1, [](Branches *b, const int& idx) -> float {
            TTLBranches* e = dynamic_cast<TTLBranches*>(b);
